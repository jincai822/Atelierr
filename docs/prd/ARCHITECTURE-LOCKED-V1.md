...
> v1.6 修订（2026-09-21 用户裁决，原教旨改造）：归档一级目录从「平台制」改「领域制」——取笔记首个中图法标签经 `scripts/dispatch/archive.py` 的 `CLC_TO_DOMAIN` 映射领域目录（career/work/health/finance/personal/…），平台（抖音/小红书/书籍）与中图法分类号降级为标签，二级目录取消；推导不出落 `personal/` 兜底。机器产出新笔记放权自动归档（产出即归档，异常留 inbox 人工兜底）。日记原教旨分层：`memory/daily-notes/YYYY/MM/`。v1.5 正文中「确认归档→移进 memory/ 平台目录」表述由本条取代。
> v1.6 增补（2026-09-22 用户裁决，方案 C）：中图法全字母覆盖——B 拆 `哲学/`（B84 心理学例外留 health），A/C/D/E/H/N/O/P/Q/S/V/X 各有主题格子；新领域目录纯中文名、按需创建；前缀回退含数字（B849→B84→B）。v1.5 增补中的「语义搜索（向量）明确不做」条款自 2026-09-19 方案三（basic-memory 记忆底座）起废止。
> v1.6 增补（2026-09-22，仪表盘豁免）：仪表盘/导航页（库根 `dashboard.md`、`主页.md`）是操作信号不是知识，置于 Obsidian 库根、`memory/` 之外——不登记、不衰减、不进搜索复习。
...
> 同日增补（三件套）：① layer 声明为 confidence 的派生视图（分桶标签，供展示与搜索过滤；不是独立信号、不驱动流程——接进流程就是重复计算）；② resurface 升级为简化 SM-2 间隔重复：复习卡「想起来了/没想起来」反馈按钮驱动每篇独立间隔（×2/÷2，1~60 天），只写 resurface.json，不进 confidence 公式；③ decay 每次运行写 state/decay-last.json 账（晨报「昨日 decay」行的数据源，供参数校准观察）。语义搜索（向量）明确不做：库太小（<百篇）关键词秒级免费够用，embedding 引入模型依赖+成本——触发条件：库 >500 篇且关键词搜索实测召回不足时重议。平面存储、sidecar 索引、无状态 confidence、机器不改写笔记等核心不变量全部保持。

---

## 文档说明

本文档是 Atelierr 系统的**官方架构规范**，当前锁定版本为 v1.6。
...
│  【2026-09-18 用户裁决：已退役】     │
│  入口收敛为 Obsidian + 飞书；归一化  │
│  由 scripts/web/integration.py 负责  │
└────────────┬─────────────────────────┘
             ↓ (读写文件系统)
┌──────────────────────────────────────┐
...
> **2026-09-18 用户裁决：本模块退役（容器已移除）。** 裁决理由：入口
> 收敛为 Obsidian + 飞书，闲置入口是认知负担。外部写入（Obsidian/
> Syncthing 落盘）的归一化职责由 `scripts/web/integration.py`
> （WebIntegration，原 FlatnotesIntegration 改名）继续承担——它是纯
> 文件系统门面，不依赖任何 Web 服务。以下小节仅作历史存档保留。
