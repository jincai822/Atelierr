...
③ Obsidian=电脑主入口[直写+大文件直接拖 attachments：按文件类型路由不认
目录、投错也能处理，唯视频必须投 媒体/]，
...
attachments/<平台>/ 下 .md（新产与 480p 视频同主名，迁移旧文与笔记同名；
原子写同名跳过；attachments 不进笔记扫描域不进 decay），卡上只留
「## 全文」链接节；搜索分「笔记/资料
全文」两组（资料组扫 attachments/**/*.md，只在显式搜索时触发）；
剪藏不动（全文笔记照旧，机器不搬）。memory/ 根=收件箱；确认后归子目录
（平台/ · 书籍/分类/ · 日记/想法/目标/）：你手动拖或点飞书 📁 按钮，
机器只在你触发时移动且衰减状态跟随；**归档规矩（09-12 用户裁决）**：
平台文件夹根目录一律**平铺**——抖音/小红书/B站/ 下直接放笔记，不再建
中图法/主题子目录（B84-心理学/ B516-德国哲学/ 两个历史目录当日已平铺
回收），分类全部交给标签（中图法+主题词）；唯一例外 书籍/ 保留类型
子目录——文件夹管「从哪来」，标签管「是什么」；搜索/衰减递归覆盖子目录
...
（每月 2 日 04:17，异常才提醒）。信噪比治理（09-12）：搜索按
confidence×来源权重排序——机器搬运全文（link/media/webclip/highlights）
降权 ×0.85 但仍可被搜到，人写笔记同热度排前；晨报复习不推机器全文
（被 [[引用]] ≥1 次的除外），复习位只留给人写笔记；**方案 C 差异衰减**
（09-12 用户特批，架构 v1.4）：link/media 转写闲置时间轴 ×3——无人碰
约 15 天进待删（人写 45 天），webclip/highlights 原速，被引用/被访问
照旧减缓与重置，公式仍是无状态纯函数（conf=0.95^(idle×source/ref)，
`decay.machine_factor` 可配）。机器产物
（摘要/清单/控制台）隔离进 `memory/系统/`（09-08）：渲染可见但不进
decay、不进反链统计。
