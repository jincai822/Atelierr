...
| `runtime.local.toml.example` | Template for the optional per-user runtime default. `scripts/atelier/atelier_runtime.py use <runtime>` writes the gitignored live file. |
...
| `model_costs.toml` | Per-1M-token USD prices by model identity, consumed by `scripts/atelier/shadow.py report` (fails closed when prices are >90 days stale). Per-user overrides in gitignored `profile/model_costs.toml`. |
| `shadow_tasks.toml` | Per-task-type verdict-token extraction rules for `scripts/atelier/shadow.py report`. |

Two pricing surfaces exist. `harness/model_costs.toml` (above) holds per-identity costs on the shadow-report path. `scripts/pricing.toml` is a separate per-provider catalog consumed only by `scripts/atelier/pricing.py` for cost estimation and future Pareto-optimal model selection. Nothing on the dispatch path loads either, so both stay out of the runtime contract.
...
```bash
python3 scripts/atelier/atelier_runtime.py run hi
python3 scripts/atelier/atelier_runtime.py use claude
python3 scripts/atelier/atelier_runtime.py run hi
```
...
`uv run scripts/atelier/render_runtime_edges.py --runtime codex --check` proves the
...
```bash
python3 scripts/atelier/harness_lint.py
python3 scripts/atelier/harness_smoke.py
```
