...
- category：从下方分类表选 1 个最贴切的类目，优先选二级类（如 B84-心理学）；二级无合适的用大类（如 B-哲学）；无法归类用 Z-综合。**类目必须带类名**（照抄表内条目如 G79自学·自我提升），禁止只给裸编号（如 G79）——裸编号无法归档，等于没分类。禁止编造表外编号，拿不准就选更宽的类。哲学内容优先细分到 B2/B5/B84 等二级类，跨多个哲学分支才用 B-哲学；教育体制/升学/专业选择归 G4，个人自我提升方法归 G79。
