...
def check_runtime_registry() -> list[Finding]:
    ...
    supporting_paths = (
        ...
        ROOT / "scripts" / "atelier" / "atelier_runtime.py",
    )
...
def check_claude_hooks() -> list[Finding]:
    ...
    required = (
        ...
        ("Stop", ("scripts/atelier/session_replay.py", "hook --runtime claude-code")),
        ("SessionEnd", ("scripts/atelier/session_replay.py", "hook --runtime claude-code")),
        ("SessionEnd", ("scripts/atelier/shadow.py", "gc")),
    )
    for event, needles in required:
        commands = commands_for(event)
        if not any(all(needle in command for needle in needles) for command in commands):
...
def check_codex_command_skills() -> list[Finding]:
    ...
    for name, entry in sorted(commands.items()):
        ...
        if "scripts/atelier.py" in text or "scripts/atelier/intent_coverage.py" in text:
            findings.append(
                Finding(
                    "ERROR",
...
def check_path_registry_drift() -> list[Finding]:
    ...
    a `paths.toml` edit + `scripts/atelier/rewrite_paths.py` invocation.
        ...
        for seg, count in sorted(unknown_literals.items()):
            findings.append(
                Finding(
                    ...
                    f"`scripts/atelier/rewrite_paths.py --templatize`, or add the "
...
def check_scripts_zk_paths() -> list[Finding]:
    ...
    # Top-level scripts/ plus scripts/atelier/ (framework code moved there);
    # `scripts/oneoff/` is gitignored (one-off migration scripts that
    # hardcode private vault content) and excluded from steady-state lint
    # coverage by convention.
    for path in sorted([*scripts_dir.glob("*.py"), *(scripts_dir / "atelier").glob("*.py")]):
        ...
        if missing or extra:
            ...
            findings.append(
                Finding(
                    ...
                    " (run `python3 scripts/atelier/harness_lint.py --fix-used-by`)",
                )
            )
        # (e) orphan
...
# Tiers that have undergone directory fission (`scripts/atelier/fission.py`,
...
def _flat_tier_glob_findings() -> list[Finding]:
    ...
    """
    findings: list[Finding] = []
    for path in sorted([*(ROOT / "scripts").glob("*.py"), *(ROOT / "scripts" / "atelier").glob("*.py")]):
...
def check_bot_trailer_banned(roots: list[str] | None = None) -> list[Finding]:
    ...
    scripts/atelier/autoevo_commit.py is the sole committer."""
    findings: list[Finding] = []
            ...
            if BANNED_BOT_TRAILER in path.read_text(encoding="utf-8", errors="ignore"):
                findings.append(
                    Finding(
                        ...
                        "bot-trailer-banned",
                        rel(path),
                        "dead Co-Authored-By bot trailer; route the commit through scripts/atelier/autoevo_commit.py",
                    )
                )
    return findings
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="scripts/atelier/harness_lint.py",
