...
Components (each skips cleanly when its substrate is unavailable):
  ...
  semantic   delegates to scripts/atelier/semantic_eval.py when the gold set and a
...
Usage: uv run scripts/atelier/eval_run.py [--json]
"""
...
def eval_semantic() -> dict:
    ...
    result = subprocess.run(
        [sys.executable, str(ROOT / "scripts" / "atelier" / "semantic_eval.py"), "--json"],
        cwd=ROOT,
        capture_output=True,
        text=True,
        timeout=1800,
