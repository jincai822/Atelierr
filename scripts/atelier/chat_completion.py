...
Two modes — pick by use case:
  ...
  STATELESS (default, no flag)
      ...
      scripts/atelier/chat_completion.py --model deepseek_pro --prompt "..."
  ...
  STATEFUL (--session FILE)
      ...
      scripts/atelier/chat_completion.py --model X --session /tmp/s.json --prompt "first"
      scripts/atelier/chat_completion.py --model X --session /tmp/s.json --prompt "second"

Other invocation flavors:

  Stdin via '-':
      echo "Hello" | scripts/atelier/chat_completion.py --model X --prompt -

  Direct flags (no --model — ad-hoc):
      scripts/atelier/chat_completion.py --endpoint https://api.example.com/v1/chat/completions \\
...
entries (written explicitly by `scripts/atelier/shadow.py log --leg native`
in-band after each native project-agent dispatch; the runtime-aware identity
comes from `scripts/atelier/shadow.py native-model` - see Pattern B in
`protocols/shadow-log.md`) carry `native`.
Cost is NOT computed at write time; `scripts/atelier/shadow.py report` derives
...
def _mirror_shadow_skeleton(event: dict) -> None:
    ...
    needed for cross-leg aggregation in `scripts/atelier/shadow.py report` plus a
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(
        prog="scripts/atelier/chat_completion.py",
    ...
    ap.add_argument(
        ...
        help=(
            ...
            "ATELIER_SHADOW_GROUP when omitted. Aggregated by `scripts/atelier/shadow.py report`."
        ),
    )
    ap.add_argument(
        "--task-type",
        ...
        help=(
            ...
            "when omitted. Used by `scripts/atelier/shadow.py report` for verdict-token "
