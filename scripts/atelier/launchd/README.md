...
| `com.atelier.semantic-index.plist` | 07:30 and 19:30 local, plus load/login catch-up | Owner-gated, offline, timeout-bounded `scripts/atelier/semantic.py index --if-stale` |
...
AWS_PROFILE=atelier-lock uv run scripts/atelier/routine_lock.py setup-table
...
`scripts/atelier/routine_audit.py`. Ordinary routines use `workspace-write`, start in
...
`scripts/atelier/harness_smoke.py`; inspect live state with
...
uv run scripts/atelier/semantic.py status --format json
...
```bash
python3 scripts/atelier/autoevo_verify.py --cycle "$(date +%Y-%m-%d)" --json
```
...
uv run scripts/atelier/routine_lock.py status autoevo-nightly
...
AWS_PROFILE=atelier-lock uv run scripts/atelier/routine_lock.py acquire autoevo-nightly --cycle test
AWS_PROFILE=atelier-lock uv run scripts/atelier/routine_lock.py release autoevo-nightly --cycle test
```
