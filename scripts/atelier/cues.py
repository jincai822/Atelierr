...
# Allow running as `uv run scripts/atelier/cues.py` from atelier root.
...
def check_recurring(ov: Path, today: date) -> tuple[Cue | None, str]:
    ...
    mute_hint = "Run `uv run scripts/atelier/recurring.py done <slug>` when complete."
    return (
        Cue(
            key="recurring",
            severity=severity,
            command_path="scripts/atelier/recurring.py",
            message=(
                f"Recurring obligations: {listing}. "
                f"`uv run scripts/atelier/recurring.py list` to see. {mute_hint}"
            ),
        ),
    ...
    *,
    ...
    gate_fixes = {
        "dirty_vault_worktree": (
            ...
            "`uv run scripts/atelier/autoevo_preflight.py --dirty-scope`"
        ),
...
def check_eval_regression(ov: Path, today: date) -> tuple[Cue | None, str]:
    ...
    return (
        Cue(
            key="eval_regression",
            severity="hard",
            command_path="scripts/atelier/eval_run.py",
            message=(
    ...
    return (
        Cue(
            ...
            message=(
                ...
                f"跑 `uv run scripts/atelier/intent_coverage.py intent-misses --propose` "
    ...
    return (
        Cue(
            ...
            message=(
                ...
                f"按协议该跑一次 meta-reflection 了 (`uv run scripts/atelier/session_stats.py` 先看数据). 现在跑吗?"
            ),
        ),
