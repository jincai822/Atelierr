...
# Python 3.10 compatibility: tomllib only exists in 3.11+
try:
    import tomllib
...
def _atelier_root() -> Path:
    """Repo root: nearest ancestor containing harness/paths.toml."""
    here = Path(__file__).resolve()
    for candidate in here.parents:
        if (candidate / "harness" / "paths.toml").exists():
            return candidate
...
def tier_files(name: str, pattern: str = "*.md") -> list[Path]:
    ...
    Tiers undergo directory fission (`scripts/atelier/fission.py`, repo-conventions
