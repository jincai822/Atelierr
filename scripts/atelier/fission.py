...
Workflow: this script only moves files. After fission, run scripts/atelier/relink.py
...
Usage:
  uv run scripts/atelier/fission.py --dir zk/wiki --axis first-letter --dry-run
  uv run scripts/atelier/fission.py --dir zk/agent-findings --axis first-letter --apply
  uv run scripts/atelier/fission.py --dir zk/reflections --axis year-month --apply
"""
...
def main() -> None:
    ...
    if args.apply:
        ...
        print("[next] run: uv run scripts/atelier/relink.py --apply", file=sys.stderr)
    else:
