...
def check_harness_lint() -> None:
    payload = json.loads(run(["scripts/atelier/harness_lint.py", "--json"]))
...
def check_paper_cache() -> None:
    ...
    for document, label in (
        ...
        expect(
            "scripts/atelier/paper_cache.py" in document,
    ...
    with tempfile.TemporaryDirectory(prefix="atelier-paper-cache-") as temp_dir:
        ...
        first = subprocess.run(
            [PYTHON, "scripts/atelier/paper_cache.py", str(source), "--json"],
            cwd=ROOT,
            capture_output=True,
            text=True,
            env=env,
        ...
        collision = subprocess.run(
            [PYTHON, "scripts/atelier/paper_cache.py", str(colliding), "--json"],
            cwd=ROOT,
            capture_output=True,
            text=True,
            env=env,
        ...
        empty = subprocess.run(
            [PYTHON, "scripts/atelier/paper_cache.py", str(empty_source), "--json"],
            cwd=ROOT,
            capture_output=True,
            text=True,
            env=env,
        ...
        rejected = subprocess.run(
            [PYTHON, "scripts/atelier/paper_cache.py", str(outside)],
            cwd=ROOT,
            capture_output=True,
            text=True,
            env=env,
...
def check_semantic_maintenance() -> None:
    ...
    plist_path = ROOT / "scripts" / "atelier" / "launchd" / "com.atelier.semantic-index.plist"
...
def check_autoevo_reliability() -> None:
    plist_path = ROOT / "scripts" / "atelier" / "launchd" / "com.atelier.autoevo-nightly.plist"
    plist = plistlib.loads(plist_path.read_bytes())
    expect(
...
def check_codex_routine_runner() -> None:
    ...
    autoevo_verifier = (ROOT / "scripts" / "atelier" / "autoevo_verify.py").read_text(
        encoding="utf-8"
    )
    required_fragments = (
    ...
    for fragment in (
        ...
        "scripts/atelier/autoevo_run.py plan",
        "scripts/atelier/autoevo_run.py outcome",
        "scripts/atelier/autoevo_run.py tombstone-check",
        "scripts/atelier/autoevo_run.py snapshot",
        "scripts/atelier/autoevo_run.py stage-merge",
        "scripts/atelier/autoevo_run.py rollback",
        "scripts/atelier/autoevo_quarantine.py update",
        "scripts/atelier/autoevo_quarantine.py insert-skipped",
        "### Sweep reports (<S>)",
    ):
        expect(
            fragment in autoevo,
    ...
    run_helper = (ROOT / "scripts" / "atelier" / "autoevo_run.py").read_text(encoding="utf-8")
    ...
    expect(
        'scripts/atelier/autoevo_commit.py' in autoevo,
...
""".lstrip(),
        ...
        audit = subprocess.run(
            [PYTHON, "scripts/atelier/routine_audit.py", "audit", "--json"],
            cwd=ROOT,
            capture_output=True,
            text=True,
            env=env,
        ...
        local = subprocess.run(
            [
                PYTHON,
                "scripts/atelier/routine_audit.py",
                "resolve",
                "sample",
                "--surface",
                "local",
...
""",
        ...
        bundle = subprocess.run(
            [
                PYTHON,
                "scripts/atelier/routine_cloud_bundle.py",
                "--output",
                str(bundle_dir),
                "--json",
            ],
    ...
    timeout = subprocess.run(
        [
            PYTHON,
            "scripts/atelier/command_timeout.py",
            "--seconds",
            "0.05",
            "--",
            PYTHON,
...
def check_routine_owner() -> None:
    owner_script = ROOT / "scripts" / "atelier" / "routine_owner.py"
    lock_script = ROOT / "scripts" / "atelier" / "routine_lock.py"
    guard_script = ROOT / "scripts" / "atelier" / "routine_prompt_guard.py"
...
def check_routine_claim() -> None:
    with tempfile.TemporaryDirectory(prefix="atelier-routine-claim-") as temp_dir:
        ...
        validated_cycle = subprocess.run(
            [
                PYTHON,
                "scripts/atelier/routine_claim.py",
                "autoevo-nightly",
                "--validate-cycle",
                "2026-07-25",
            ],
        ...
        written = subprocess.run(
            [PYTHON, "scripts/atelier/routine_claim.py", "sample", "--cycle", "cycle"],
            cwd=ROOT,
            input=valid_content,
            capture_output=True,
            text=True,
        ...
        watch.write_text('[coordination]\nbackend = "none"\n', encoding="utf-8")
        first = subprocess.run(
            [PYTHON, "scripts/atelier/routine_lock.py", "acquire", "local", "--cycle", "cycle"],
            cwd=ROOT,
            capture_output=True,
            text=True,
            env=env,
        ...
        recovered = subprocess.run(
            [
                PYTHON,
                "scripts/atelier/routine_lock.py",
                "recover",
                "local",
                "--cycle",
                "cycle",
...
def check_runtime_selector() -> None:
    status = json.loads(run(["scripts/atelier/atelier_runtime.py", "status", "--json"]))
    expect(
    ...
    overridden = json.loads(
        run(
            ["scripts/atelier/atelier_runtime.py", "resolve", "--json"],
    ...
    neutral = subprocess.run(
        [PYTHON, "scripts/atelier/shadow.py", "native-model", "--agent", "thinker"],
        cwd=ROOT,
        capture_output=True,
        text=True,
        env=neutral_env,
...
def check_codex_intent_hook() -> None:
    with tempfile.TemporaryDirectory(prefix="atelier-codex-hook-") as temp_dir:
        ...
        exact_output = run(
            ["scripts/atelier/intent_coverage.py", "intent-hook", "--runtime", "codex"],
...
def check_claude_intent_hook() -> None:
    with tempfile.TemporaryDirectory(prefix="atelier-claude-hook-") as temp_dir:
        ...
        for prompt in (
            ...
            fallback_output = run(
                [
                    "scripts/atelier/intent_coverage.py",
                    "intent-hook",
                    "--runtime",
                    "claude-code",
                ],
