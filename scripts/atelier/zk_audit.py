...
CLI:
    uv run scripts/atelier/zk_audit.py            human report
    uv run scripts/atelier/zk_audit.py --json     machine-readable output

Exit code: 0 always (audit is advisory; user decides what to consolidate).
2 only on IO error (e.g., $OV is missing).

Design notes:
  - Stdlib only, mirrors scripts/atelier/privacy_check.py and scripts/atelier/lint.py style.
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="scripts/atelier/zk_audit.py",
