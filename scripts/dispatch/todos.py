...
class TodoDispatcher:
    ...
    def _chat_todos(self, text: str, api_key: str) -> List[Dict[str, Any]]:
        ...
        """
        prompt = (
            # 基准日期必须先给——不给"今天几号"，"明天/周五前"无从推算，
            # due 必然丢空（2026-09-15 实证："明天把 dfmea 看完"产出无日期）
            f"今天日期 {datetime.now().astimezone().strftime('%Y-%m-%d %A')}；"
            "相对时间词（今天/明天/后天/周X前/下周）一律按今天推算成绝对日期。"
