...
- 2026-09-22 方案 C（用户拍板）：中图法全字母覆盖——B 拆出
  哲学/（B84 心理学例外留 health），A/C/D/E/H/N/O/P/Q/S/V/X
  各有主题格子（纯中文名，同日拍板），目录按需创建，不再挤进
  personal；
...
#: 沿用；2026-09-22 方案 C（用户拍板）：全字母覆盖，B 拆 哲学/
#: （B84 心理学特例留 health），其余字母各有主题格子——新目录用
#: 纯中文名（同日用户拍板：Obsidian 里看着顺眼），按需创建。
#: 匹配时前缀逐级回退：先缩数字（B849→B84），再缩字母（TN→T）。
CLC_TO_DOMAIN = {
    "B84": "health",  # 心理学 → 稳定内核（B 下唯一特例，数字回退命中）
    "B": "哲学",      # 哲学·宗教（2026-09-22 从 health 拆出）
    "R": "health",   # 医药卫生
