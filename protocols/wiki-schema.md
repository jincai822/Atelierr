...
The `## Summary`, `## Claims`, and `## Revision Log` headings are required. The filename carries the title (no H1 inside the file). Topic tags (regular Obsidian-style hashtags) are allowed but not required and play no role in the trust engine.
...
- Shadow filename matches the English source filename exactly (no H1 inside either file)
