...
| **Semantic index (LanceDB)** | derived-cache | $OV → cache (maintained by owner-gated `semantic.py index --if-stale`) | none (rebuildable) | `semantic.py status` reports corpus drift or an unreadable index; full rebuild is manual recovery | Index lives at `~/.cache/atelier/lance/`; never committed |
