...
3. Complexity audit: Count protocols, agents, frameworks. Compare to the trailing-30-day session count (`uv run scripts/atelier/session_stats.py --json`). If protocols exceed 1.5x that count, or `harness_lint` fires `prose-budget`, trigger a pruning review before any additive changes, because the system's default failure mode is monotonic growth.
