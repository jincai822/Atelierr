...
勾成 `[x]`，下一轮同步（15 分钟内）勾中条目自动转为 wiki/ 根层的「摘录卡」
（type: Excerpt，from 指回清单+页码——勾选即沉淀为永久资产，不经 memory
待确认中转；周日提炼时把它改写为自己的 concept 卡并互链）。也可手动单条处理
（需 `DEEPSEEK_API_KEY`）：
...
确认过的笔记（无「待确认」标签）可以按你的体系整理：在 Obsidian 里
直接把它**拖进 memory/ 下的子目录**——机器**永远不会**替你移动或
删除笔记文件，但系统会跟上你的手动整理：

- **一级目录**：平台文件夹，如 `抖音/`、`小红书/`、`飞书/`、`媒体/`；
  外加 `书籍/`（你的书，下按分类标签建二级目录）、`日记/`（日期
  笔记、周记）、`想法/`（闪念、灵感、随手记）、`目标/`（目标、
  计划、复盘）；
- **二级目录**：`书籍/` 下按中图法分类标签建，如 `书籍/B84-心理学/`、
  `书籍/I-文学/`（frontmatter `tags` 里的分类标签即目录名）；平台内容
  里的书籍/专题类也可同样再分一层，如 `抖音/B84-心理学/`；
- 移入后由 watcher（15 分钟定时班次或常驻监听）自动迁移 sidecar
  条目：按 frontmatter `id` 认出"老文件的新位置"，**衰减状态原样
  保留**（created/last_accessed/confidence/层级不丢），新路径立刻
  重新参与搜索/衰减/复习/摘要；
- 飞书卡片确认回调同样覆盖子目录：笔记先归档再点「✅ 确认」也能
  命中；同名笔记出现在多个目录时会提示到 Obsidian 处理，不会乱改；
- `wiki/`（摘录卡）、`attachments/`（附件）、`trash/`（回收站）、
  `templates/`（Obsidian 模板）是机器专用目录，永远不参与笔记扫描，
  别把普通笔记放进去。

链接/OCR 笔记刚转写完成时，飞书卡片正文会带一行
`建议归档：抖音/B84-心理学/` 之类的提示（从笔记 source 与 tags 里
推断，取不到就省略）——照着建目录拖进去即可。
