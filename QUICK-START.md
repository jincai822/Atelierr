...
- **一级目录**：平台文件夹，如 `抖音/`、`小红书/`、`飞书/`；
- **二级目录**（可选）：书籍/专题类目，按中图法分类标签建，如
  `抖音/B84-心理学/`（frontmatter `tags` 里的 `B84-心理学` 即对应
  目录名）；
...
- `wiki/`（摘录卡）、`attachments/`（附件）、`trash/`（回收站）是
  机器专用目录，永远不参与笔记扫描，别把普通笔记放进去。
